...
int getCost(int cardNumber)
{
  ...
  return -1;
	...
	int adventurerf(int drawntreasure, int temphand[], int currentPlayer, struct gameState *state) {
	int z = 0;
	int cardDrawn;
    ...
    int council_roomf(struct gameState *state, int currentPlayer, int handPos)
	{
		int i;
      //+4 Cards
      for (i = 0; i < 4; i++)
	...
	int minef(struct gameState *state, int currentPlayer, int choice2, int handPos, int choice1)
	{
	int j;
	int i;
	...
	int feastf(int currentPlayer, struct gameState *state, int choice1, int temphand[])
	{
	int i;
	int x;
	...
	int remodelf(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos)
	{
		int j;
		int i;
      j = state->hand[currentPlayer][choice1];  //store card we will trash
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
 ...
 /* MUTANT (rep_op) */                        else if (tributeRevealedCards[i] == estate || tributeRevealedCards[i] == duchy || tributeRevealedCards[i] == province || tributeRevealedCards[i] == gardens || tributeRevealedCards[i] != great_hall) {//Victory Card Found
